	...
	<div class="content">
		...
		<div class="right">
			<div class="top">
				...
				<div class="top-item">
					<a @click="jumpPage()"><span class="top-text">登录</span></a>
			...
			<div class="form-wrappepr">
				<input type="text" class="inputs userId" v-model="stuId" name="stuId" placeholder="请输入账号"
					required="required" title="不能为空">
				<input type="text" class="inputs username" v-model="username" name="username" placeholder="输入你的昵称"
					required="required" title="不能为空">
				<input type="password" class="inputs pwd" v-model="password" name="password" placeholder="密码"
					required="required" title="不能为空">
				<input type="password" class="inputs pwd" v-model="rightpassword" name="rightpassword"
					placeholder=" 确认密码" required="required" title="不能为空">
				<span class="tips">Recovery Password</span>
				<button type="submit" @click="reg">Sigin In</button>
			</div>
		</div>
	</div>
</template>
<script src="../../assets/js/jquery-3.5.1"></script>
<script>
	export default {
		name: "Register",
		data() {
		...
		methods: {
			reg() {
				$.ajax({
					// url: "http://127.0.0.1:8080/registerStu",
					type: "POST",
					data: { //提交的数据是什么
						stuId: this.stuId,
					...
					async: true, //默认为true
					success: function(backMsg) { //提交成功还是失败返回的函数
						if (backMsg.id == 0) {
							// 登录成功
							alert("注册成功")
							console.log(backMsg.message);
							setTimeout(go, 1000);
...
<style>
	* {
	...
	//
	.content .right .form-wrappepr .userId {
		display: block;
		width: 100%;
		height: 70px;
		margin: 30px 0;
