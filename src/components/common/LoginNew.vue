	...
	<div class="content">
		...
		<div class="right">
			<div class="top">
				...
				<div class="top-item">
					<a @click="jumpPage()"><span class="top-text">注册</span></a>
				</div>
				<div class="top-item">
			...
			<div class="form-wrappepr">
				<h1>Login</h1>
				<input type="text" class="inputs user" name="username" v-model="username" placeholder="输入您的账号"
				 required >
				<input type="password" class="inputs pwd" name="password" v-model="password" placeholder="输入您的密码"
        required>
...
<script>
	export default {
		...
		methods: {
			login() {
								this.$axios.post(this.$httpUrl+'/user/login' , this.$qs.stringify({
									...
									if (resData != null) {
										window.sessionStorage.setItem("userData",JSON.stringify(resData))
										this.$router.push({
										  path: '/Home'
										})
